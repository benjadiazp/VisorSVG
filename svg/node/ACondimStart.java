/* This file was generated by SableCC (http://www.sablecc.org/). */

package svg.node;

import svg.analysis.*;

@SuppressWarnings("nls")
public final class ACondimStart extends PStart
{
    private TSvg1 _svg1_;
    private PDimensiones _dimensiones_;
    private TMayus _mayus_;
    private PFigura _figura_;
    private TSvg2 _svg2_;

    public ACondimStart()
    {
        // Constructor
    }

    public ACondimStart(
        @SuppressWarnings("hiding") TSvg1 _svg1_,
        @SuppressWarnings("hiding") PDimensiones _dimensiones_,
        @SuppressWarnings("hiding") TMayus _mayus_,
        @SuppressWarnings("hiding") PFigura _figura_,
        @SuppressWarnings("hiding") TSvg2 _svg2_)
    {
        // Constructor
        setSvg1(_svg1_);

        setDimensiones(_dimensiones_);

        setMayus(_mayus_);

        setFigura(_figura_);

        setSvg2(_svg2_);

    }

    @Override
    public Object clone()
    {
        return new ACondimStart(
            cloneNode(this._svg1_),
            cloneNode(this._dimensiones_),
            cloneNode(this._mayus_),
            cloneNode(this._figura_),
            cloneNode(this._svg2_));
    }

    @Override
    public void apply(Switch sw)
    {
        ((Analysis) sw).caseACondimStart(this);
    }

    public TSvg1 getSvg1()
    {
        return this._svg1_;
    }

    public void setSvg1(TSvg1 node)
    {
        if(this._svg1_ != null)
        {
            this._svg1_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._svg1_ = node;
    }

    public PDimensiones getDimensiones()
    {
        return this._dimensiones_;
    }

    public void setDimensiones(PDimensiones node)
    {
        if(this._dimensiones_ != null)
        {
            this._dimensiones_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._dimensiones_ = node;
    }

    public TMayus getMayus()
    {
        return this._mayus_;
    }

    public void setMayus(TMayus node)
    {
        if(this._mayus_ != null)
        {
            this._mayus_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._mayus_ = node;
    }

    public PFigura getFigura()
    {
        return this._figura_;
    }

    public void setFigura(PFigura node)
    {
        if(this._figura_ != null)
        {
            this._figura_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._figura_ = node;
    }

    public TSvg2 getSvg2()
    {
        return this._svg2_;
    }

    public void setSvg2(TSvg2 node)
    {
        if(this._svg2_ != null)
        {
            this._svg2_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._svg2_ = node;
    }

    @Override
    public String toString()
    {
        return ""
            + toString(this._svg1_)
            + toString(this._dimensiones_)
            + toString(this._mayus_)
            + toString(this._figura_)
            + toString(this._svg2_);
    }

    @Override
    void removeChild(@SuppressWarnings("unused") Node child)
    {
        // Remove child
        if(this._svg1_ == child)
        {
            this._svg1_ = null;
            return;
        }

        if(this._dimensiones_ == child)
        {
            this._dimensiones_ = null;
            return;
        }

        if(this._mayus_ == child)
        {
            this._mayus_ = null;
            return;
        }

        if(this._figura_ == child)
        {
            this._figura_ = null;
            return;
        }

        if(this._svg2_ == child)
        {
            this._svg2_ = null;
            return;
        }

        throw new RuntimeException("Not a child.");
    }

    @Override
    void replaceChild(@SuppressWarnings("unused") Node oldChild, @SuppressWarnings("unused") Node newChild)
    {
        // Replace child
        if(this._svg1_ == oldChild)
        {
            setSvg1((TSvg1) newChild);
            return;
        }

        if(this._dimensiones_ == oldChild)
        {
            setDimensiones((PDimensiones) newChild);
            return;
        }

        if(this._mayus_ == oldChild)
        {
            setMayus((TMayus) newChild);
            return;
        }

        if(this._figura_ == oldChild)
        {
            setFigura((PFigura) newChild);
            return;
        }

        if(this._svg2_ == oldChild)
        {
            setSvg2((TSvg2) newChild);
            return;
        }

        throw new RuntimeException("Not a child.");
    }
}
